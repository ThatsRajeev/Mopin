...
function FoodItemCard(props) {
  const isChefCard = props.cardType === "chef";

  return (
    <div className={`card-container ${isChefCard ? "chefCard-container" : "food-container"}`}>
      ...
      <div className="food-info">
        ...
        <div className={`sub-info ${isChefCard ? "chef-info" : ""}`}>
          <p>Feeds: {props.feeds}</p>
          <span className="span">{isChefCard ? "•" : "|"}</span>
          ...
          {isChefCard ? (
            <span>({parseInt(props.noOfOrders / 5, 10) * 5}+)</span>
          ) : (
            <>
